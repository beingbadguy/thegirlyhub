...
const ProductPage = () => {
  ...
  const fetchProductBySlug = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`/api/product/${encodeURIComponent(slug as string)}`);
      const fetchedProduct = response.data.product;
      setProduct(fetchedProduct);

      // Default selected image
      const primaryImage = fetchedProduct.images && fetchedProduct.images.length > 0
        ? fetchedProduct.images[0]
        : fetchedProduct.image;
      setSelectedImage(primaryImage);

      // Default variants selection (Sizes)
      if (fetchedProduct.variants?.sizes?.length > 0) {
        setSize(fetchedProduct.variants.sizes[0]);
      } else {
        const cat = fetchedProduct.category;
      ...
      // Default variants selection (Colors - Only set if explicitly defined)
      if (fetchedProduct.variants?.colors?.length > 0) {
        setColor(fetchedProduct.variants.colors[0]);
      } else {
        setColor("");
      }

      // Check review eligibility
      checkEligibility(fetchedProduct._id);
    } catch (error: unknown) {
    ...
    return (
      <div className="flex min-h-[70vh] items-center justify-center bg-neutral-50">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-neutral-500 font-serif animate-pulse">Loading product specifications...</p>
        </div>
      </div>
    );
  }
  ...
  // Colors list: Only show colors if they are explicitly added on the product variants
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F9] px-4 py-6 md:px-8 font-sans text-neutral-900">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2 text-xs font-semibold tracking-wide text-neutral-400">
        ...
        <span>/</span>
        {/* <span className="cursor-pointer transition-colors hover:text-neutral-800" onClick={() => router.push("/product")}>
          Products
        </span> */}
        {/* <span>/</span> */}
      ...
      <div className="grid grid-cols-1 gap-6 lg:gap-12 lg:grid-cols-12 max-w-7xl mx-auto bg-white rounded-3xl p-4 md:p-8 border border-neutral-100 shadow-sm">
        ...
        <div ref={buySectionRef} className="lg:col-span-6 flex flex-col justify-between space-y-6">
          <div className="space-y-4">
            ...
            {/* Colors variant swatches - Only rendered if colors list exists */}
            {showColorSelector && (
              <div className="space-y-2 py-2">
                <span className="text-xs font-bold uppercase tracking-wider text-neutral-400">
                  Choose Color
      ...
      <div id="reviews-section" className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
        ...
        {/* Write a Review Block (Restricted) */}
